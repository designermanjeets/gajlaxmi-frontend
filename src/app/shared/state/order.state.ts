...
export class OrderState {
  ...
  rePayment(ctx: StateContext<OrderStateModel>, action: RePayment) {
    return this.orderService.rePayment(action.payload).pipe(
      tap({
        next: result => {
          if((action.payload.payment_method == 'cod' || action.payload.payment_method == 'bank_transfer' || action.payload.payment_method == 'gajlaxmi_neo') && !result.is_guest) {
            this.router.navigateByUrl(`/account/order/details/${result.order_number}`);
          } else if((action.payload.payment_method == 'cod' || action.payload.payment_method == 'bank_transfer' || action.payload.payment_method == 'gajlaxmi_neo') && result.is_guest) {
